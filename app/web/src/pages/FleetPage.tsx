...
export function FleetPage() {
  ...
  const opts = useMemo(() => {
    const teams = new Set<string>(), trigs = new Set<string>(), conns = new Set<string>(), owners = new Set<string>();
    (routines ?? []).forEach((r) => { if (r.team) teams.add(r.team); if (r.owner) owners.add(r.owner); r.triggers.forEach((t) => trigs.add(t)); r.connectors.forEach((c) => conns.add(c)); });
    return { teams: [...teams], trigs: [...trigs], conns: [...conns], owners: [...owners].sort() };
  }, [routines]);
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-6 font-sans text-fg animate-fade-up">
      ...
      <div className="mb-3.5 flex flex-wrap items-center gap-[9px]">
        ...
        {(team || owner || trig || conn || q) && (
          <button onClick={() => { setTeam(''); setOwner(''); setTrig(''); setConn(''); setQ(''); }} className="font-mono text-[11px] text-dim hover:text-fg">clear</button>
        )}
      ...
      <div className="overflow-hidden rounded-xl border border-line bg-surface">
        ...
        {!routines ? (
          ...
          [...new Set([...list].sort((a, b) => a.team.localeCompare(b.team)).map((r) => r.team))].map((tm) => {
            const rs = list.filter((r) => r.team === tm);
            return (
              <div key={tm || '—'}>
                ...
                {rs.map((r) => <FleetRow key={r.slug} r={r} />)}
              </div>
            );
          })
        ) : (
          list.map((r) => <FleetRow key={r.slug} r={r} />)
        )}
      </div>
    </div>
  );
