// A fresh store ships with a real, runnable starter fleet — the same definitions the
// "Load examples" button installs (samples.js), nothing fabricated.
...
function insertRoutine(db, r, ord, repo) {
  ...
  db.prepare(
    `INSERT INTO routines
      (slug,name,summary,owner,team,triggers,connectors,state,last_ago,last_status,next,success,spend,enabled,avg,av_color,initials,ord,prompt,model,repo,branch,chain,schedule,filters,reactions,effort,memory,concurrency)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(
