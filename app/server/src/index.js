...
const shapeRoutine = (r) => {
  ...
  return {
    ...
    reviewStatus: r.review_status || '', reviewedBy: r.reviewed_by || '', reviewedAgo: r.reviewed_at ? relTime(r.reviewed_at) : '', gateReview: !!r.gate_review, tier: r.tier || 'standard', escalation: r.escalation || '', links: (() => { try { return JSON.parse(r.links || '[]'); } catch { return []; } })(), sunsetAt: r.sunset_at || 0, sunsetOverdue: !!(r.sunset_at && r.enabled && r.sunset_at < now()),
